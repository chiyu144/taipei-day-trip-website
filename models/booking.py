...
def query_booking(cursor, member_id):
  ...
  for booking in bookings:
    booking_dct = {}
    attraction_dct = dict(zip((columns[0], columns[1], columns[2], columns[3]),
...
def booking_schedule(cursor, member_id, attraction_id, date, time, price):
  cursor.execute('SELECT count(attraction_id) FROM booking WHERE member_id = %s AND attraction_id = %s',
                 (member_id, attraction_id))
